...
where start is the start position on x-axis of the outline, end is the end position on x-axis
...
public class Solution {
    ...
    public List<List<Integer>> buildingOutline(int[][] buildings) {
        ...
        for (int[] point : points) {
            if (point[1] > 0) {  // 如果是起始点，则将其添加到 maxHeap 中
                maxHeap.offer(point[1]);
            } else { // 如果是终止点，则将其从 maxHeap 中移除
                maxHeap.remove(-point[1]);
            }
            // 获取当前的最高高度
            int curr = maxHeap.isEmpty() ? 0 : maxHeap.peek();
            // 将当前的最高高度 与 上一个状态的最高高度 相比，如果高度发生变化，则记录下来
            // 并更新 上一个状态
            if (prev != curr) {
                // 跳过高度为 0 的建筑
                if (prev != 0) {
                    rst.add(new ArrayList<Integer>(Arrays.asList(start, point[0], prev)));
                }
                prev = curr;
                start = point[0];
            }
...
public class Solution {
    ...
    public List<List<Integer>> buildingOutline(int[][] buildings) {
        ...
        // 开始扫描，遍历所有的端点
        for (int[] point : points) {
            if (point[1] > 0) {  // 如果是起始点，则将其添加到 TreeMap 中，出现次数 +1
                map.put(point[1], map.getOrDefault(point[1], 0) + 1);
            } else { // 如果是终止点
                int count = map.get(-point[1]);
                if (count == 1) { // 当出现次数只有 1 次时，直接 remove
                    map.remove(-point[1]);
                } else { // 不止一次的话，出现次数-1
