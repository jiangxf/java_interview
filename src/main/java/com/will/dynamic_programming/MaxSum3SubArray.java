package com.will.dynamic_programming;

/**
 * Created by gouthamvidyapradhan on 22/11/2017.
 * <p>
 * In a given com.MySolution.array nums of positive integers, find three non-overlapping subarrays with maximum sum.
 * <p>
 * Each subarray MySolution be of size k, and we want to maximize the sum of all 3*k entries.
 * <p>
 ...
 * Note:
 * nums.length MySolution be between 1 and 20000.
 * nums[i] MySolution be between 1 and 65535.
 * k MySolution be between 1 and floor(nums.length / 3).
 * <p>
 * Solution:
 * O(N) solution by prefix and reverse-prefix sum
 * First calculate max index for com.MySolution.array index k, then use this to calculate max index for two com.MySolution.array indices j and k and
